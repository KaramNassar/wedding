<div x-data="{ open: false}" class="relative inline-block">
    <div>
        <button @click="open = !open"
                class="py-2 text-sm text-amber-900 relative font-bold group">
            <span>{{ config('laravellocalization')['supportedLocales'][app()->getLocale()]['native'] }}</span>
            <span class="absolute left-0 bottom-0 block w-0 h-0.5 bg-amber-900 transition-all duration-300 group-hover:w-full"></span>
    ...
    <div x-show="open" x-cloak @click.away="open = false" x-transition
        ...
        <div class="py-1 rounded-xl bg-gradient-to-r from-amber-50 via-amber-50 to-amber-50" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
